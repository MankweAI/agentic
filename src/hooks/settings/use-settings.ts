...
export const useSettings = (id: string) => {
  ...
  const onUpdateSettings = handleSubmit(async (values) => {
    ...
    if (values.welcomeMessageOutside) {
      const message = await onUpdateWelcomeMessageOutside(
        values.welcomeMessageOutside,
        id
      );
      if (message) {
        toast({
    ...
    if (values.agentName) {
      const message = await onUpdateAgentName(values.agentName, id);
      if (message) {
        toast({
          title: "Success",
          description: message.message,
